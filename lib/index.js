...
async function setAntiBot(groupId, type, action) {
    try {
        ...
        if (!data.antibot[groupId]) data.antibot[groupId] = { bots: [] };
        
        data.antibot[groupId] = {
            enabled: type === 'on',
            action: action || 'delete',
            bots: data.antibot[groupId].bots || []
        };
...
async function addBotToAntiBot(groupId, botJid) {
    try {
        const data = loadUserGroupData();
        if (!data.antibot) data.antibot = {};
        if (!data.antibot[groupId]) data.antibot[groupId] = { enabled: false, action: 'delete', bots: [] };
        if (!data.antibot[groupId].bots) data.antibot[groupId].bots = [];
        if (!data.antibot[groupId].bots.includes(botJid)) {
            data.antibot[groupId].bots.push(botJid);
            saveUserGroupData(data);
        }
        return true;
    } catch (error) {
        console.error('Error adding bot to antibot:', error);
        return false;
    }
}

async function removeBotFromAntiBot(groupId, botJid) {
    try {
        const data = loadUserGroupData();
        if (data.antibot && data.antibot[groupId] && data.antibot[groupId].bots) {
            const index = data.antibot[groupId].bots.indexOf(botJid);
            if (index !== -1) {
                data.antibot[groupId].bots.splice(index, 1);
                saveUserGroupData(data);
            }
        }
        return true;
    } catch (error) {
        console.error('Error removing bot from antibot:', error);
        return false;
    }
}
